...
After running `dart run build_runner clean && dart run build_runner build --delete-conflicting-outputs`, JAO generates:
...
<Warning>
Always run `dart run build_runner clean && dart run build_runner build --delete-conflicting-outputs` after modifying models to regenerate the query classes.
</Warning>
